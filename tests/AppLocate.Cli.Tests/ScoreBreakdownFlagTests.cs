...
public class ScoreBreakdownFlagTests
{
    ...
    public void Json_Includes_Breakdown_When_Flag()
    {
        ...
        if (code == 0)
        {
            ...
            if (arr.Count == 0) return; // environment may have no matches
            // At least one element should contain a 'breakdown' object with expected numeric properties
            bool anyWith = arr.Any(e => e.TryGetProperty("breakdown", out var b) && b.ValueKind == JsonValueKind.Object && b.TryGetProperty("total", out _));
