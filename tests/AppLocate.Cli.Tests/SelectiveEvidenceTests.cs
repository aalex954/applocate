    ...
    public class SelectiveEvidenceTests {
        ...
        public void EvidenceKeysImplicitlyEnableEvidence() {
            ...
            if (code == 0) {
                ...
                if (hits.Count == 0) {
                    return; // environment may lack sample apps
                }

                var anyEvidenceKey = hits.Any(h => h.TryGetProperty("evidence", out var ev) && ev.ValueKind == JsonValueKind.Object && ev.EnumerateObject().Any(p => p.NameEquals("Shortcut") || p.NameEquals("ProcessId")));
