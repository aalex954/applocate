...
public class CliOptionsTests
{
    ...
    public async Task JsonOption_ParsesAndReturnsSuccessOrNoHits()
    {
        ...
        var code = await Program.RunAsync(new[]{"vscode","--json","--limit","1"});
    ...
    {
        var code = await Program.RunAsync(new[]{"testapp","--limit","2"});
        Assert.True(code == 0 || code == 1);
    }
    ...
    {
        var code = await Program.RunAsync(new[]{"foo","--confidence-min","2"}); // invalid >1
        Assert.Equal(2, code);
    }
    ...
    {
        var code = await Program.RunAsync(new[]{"--json"});
        Assert.Equal(2, code);
    }
}
