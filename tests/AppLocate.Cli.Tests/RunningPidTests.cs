...
namespace AppLocate.Cli.Tests {
    public class RunningPidTests {
        ...
        public void RunningFlag_DoesNotError() {
            var (code, _, err) = Run("code", "--json", "--running", "--limit", "5");
        ...
        public void PidFlag_AddsProcessPath() {
            ...
            if (code == 0) {
                ...
                var hasProcExe = hits.Any(h => h.GetProperty("path").GetString()!.Equals(current.MainModule?.FileName, StringComparison.OrdinalIgnoreCase));
        ...
        public void InvalidPid_Exit2() {
            var (code, _, err) = Run("code", "--json", "--pid", "-5");
            Assert.Equal(2, code);
