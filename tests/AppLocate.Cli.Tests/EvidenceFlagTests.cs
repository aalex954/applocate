        ...
        public void EvidenceFlag_AddsEvidenceObjects() {
            ...
            if (codeWith == 0 && codeNo == 0) {
                ...
                if (hitsWith.Count > 0 && hitsNo.Count > 0) {
                    ...
                    var anyEvidenceWith = hitsWith.Any(h => h.TryGetProperty("evidence", out var ev) && ev.ValueKind != JsonValueKind.Null && ev.EnumerateObject().Any());
                    var anyEvidenceWithout = hitsNo.Any(h => h.TryGetProperty("evidence", out var ev) && ev.ValueKind != JsonValueKind.Null && ev.EnumerateObject().Any());
