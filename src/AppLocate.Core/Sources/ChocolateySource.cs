...
public sealed class ChocolateySource : ISource {
    ...
    public async IAsyncEnumerable<AppHit> QueryAsync(
        ...
        foreach (var packageDir in packageDirs) {
            ...
            if (packageNameLower is "_processed" or ".chocolatey") continue;
            ...
            if (Directory.Exists(shimsDir)) {
                foreach (var shimExe in SafeGetFiles(shimsDir, "*.exe")) {
                    ...
                    var shimGui = shimExe + ".gui";
                    if (File.Exists(shimIgnore)) continue;
    ...
    private static bool MatchesQuery(string nameLower, string normalizedQuery, string[] queryTokens) {
        if (nameLower.Contains(normalizedQuery)) return true;
        if (queryTokens.All(t => nameLower.Contains(t))) return true;
        if (normalizedQuery.Contains(nameLower)) return true;
        return false;
    }
