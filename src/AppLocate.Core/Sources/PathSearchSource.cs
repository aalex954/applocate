...
namespace AppLocate.Core.Sources {
    ...
    public sealed class PathSearchSource : ISource {
        ...
        public async IAsyncEnumerable<AppHit> QueryAsync(string query, SourceOptions options, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct) {
            ...
            if (norm.Contains(' ')) {
                _ = aliasForms.Add(norm.Replace(" ", string.Empty));
            }
            ...
            if (norm.Contains(' ') && !aliasForms.Contains(norm.Replace(' ', '-'))) {
                _ = aliasForms.Add(norm.Replace(' ', '-'));
            }
            ...
            if (!query.Contains(' ')) {
                whereCandidates.Add(query);
            }
            else {
                var collapsed = query.Replace(" ", string.Empty);
                if (collapsed.Length > 2) {
                    whereCandidates.Add(collapsed);
                }

                if (query.Contains(' ') && !query.Contains('-')) {
                    var hyphen = query.Replace(' ', '-');
                    if (hyphen.Length > 2) {
                        whereCandidates.Add(hyphen);
                    }
                }
            }
                ...
                try {
                    ...
                    if (finished) {
                        while (!p.StandardOutput.EndOfStream) {
                            ...
                            buffered ??= [];
                            buffered.Add(new AppHit(HitType.Exe, scope, line, null, PackageType.EXE, [Name], 0, evidence));
                            var dir = Path.GetDirectoryName(line);
                            if (!string.IsNullOrEmpty(dir) && yielded.Add(dir + "::install")) {
                                buffered.Add(new AppHit(HitType.InstallDir, scope, dir!, null, PackageType.EXE, [Name], 0, evidence));
                            }
                        }
                    }
            ...
            foreach (var dir in parts) {
                ...
                foreach (var file in files) {
                    ...
                    if (!options.Strict) {
                        ...
                        match = pathTokens.All(name.Contains);
                    }
                    if (!match) {
                        continue;
                    }
                    ...
                    buffered ??= [];
                    buffered.Add(new AppHit(HitType.Exe, scope, file, null, PackageType.EXE, [Name], 0, evidence));
                    ...
                    if (!string.IsNullOrEmpty(dirName) && yielded.Add(dirName + "::install")) {
                        ...
                        buffered.Add(new AppHit(HitType.InstallDir, scope, dirName!, null, PackageType.EXE, [Name], 0, dirEvidence));
                    }
                }
                if (buffered != null) {
                    foreach (var h in buffered) {
            ...
            if (!ct.IsCancellationRequested && (pathTokens.Length > 1 || norm.Contains('-'))) {
                ...
                if (!string.IsNullOrEmpty(pf) && pf != pf86) {
                    roots.Add(pf!);
                }
                ...
                foreach (var variant in aliasForms) {
                    ...
                    foreach (var root in roots) {
                        ...
                        foreach (var cand in candidates.Distinct(StringComparer.OrdinalIgnoreCase)) {
                            ...
                            if (!cand.EndsWith('.' + "exe", StringComparison.OrdinalIgnoreCase)) {
                                continue;
                            }
                            ...
                            yield return new AppHit(HitType.Exe, scope, cand, null, PackageType.EXE, [Name], 0, evidence);
                            var dirOut = Path.GetDirectoryName(cand);
                            if (!string.IsNullOrEmpty(dirOut) && yielded.Add(dirOut + "::install")) {
                                yield return new AppHit(HitType.InstallDir, scope, dirOut!, null, PackageType.EXE, [Name], 0, evidence);
                            }
                        }
                    }
                }
        ...
        private static Scope InferScope(string path) {
            try {
                ...
                return lower.Contains("\\users\\") ? Scope.User : Scope.Machine;
            }
