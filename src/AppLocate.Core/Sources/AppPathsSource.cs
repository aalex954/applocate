            ...
            foreach (var root in roots) {
                ...
                foreach (var sub in subs) {
                    ...
                    if (!string.IsNullOrWhiteSpace(exePath) && File.Exists(exePath)) {
                        yield return new AppHit(HitType.Exe, scope, exePath!, null, PackageType.EXE, [Name], 0, evidence);
                        var dir = Path.GetDirectoryName(exePath);
                        if (!string.IsNullOrEmpty(dir) && Directory.Exists(dir)) {
                            yield return new AppHit(HitType.InstallDir, scope, dir!, null, PackageType.EXE, [Name], 0, evidence);
                        }
                    }
                    if (!string.IsNullOrWhiteSpace(pathDir) && Directory.Exists(pathDir)) {
                        yield return new AppHit(HitType.InstallDir, scope, pathDir!, null, PackageType.EXE, [Name], 0, evidence);
                    }
                }
            }
        }
