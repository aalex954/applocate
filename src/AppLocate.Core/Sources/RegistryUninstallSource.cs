    ...
    public sealed class RegistryUninstallSource : ISource {
                ...
                using (rk) {
                    ...
                    foreach (var sub in subNames) {
                        ...
                        try {
                            ...
                            if (options.Strict) {
                                if (!AllTokensPresent(dnLower!, queryTokens)) {
                                    continue;
                                }
                            }
                            else {
                                if (!dnLower!.Contains(normalizedQuery) && !keyLower.Contains(normalizedQuery)) {
                                    continue;
                                }
                            }
                            ...
                            var exeCandidate = PathUtils.NormalizePath(ParseExeFromDisplayIcon(displayIconRaw));
                            ...
                            if (options.IncludeEvidence) {
                                ...
                                if (windowsInstaller) {
                                    evidence[EvidenceKeys.WindowsInstaller] = "1";
                                }

                                if (installLocation != null) {
                                    evidence[EvidenceKeys.HasInstallLocation] = "true";
                                }

                                if (!string.IsNullOrEmpty(displayIconRaw)) {
                                    evidence[EvidenceKeys.HasDisplayIcon] = "true";
                                }
                            }

                            if (installLocation != null && PathIsPlausibleDir(installLocation)) {
                                sink.Add(new AppHit(HitType.InstallDir, scope, installLocation, version, pkgType, [Name], 0, evidence));
                            }
                            if (exeCandidate != null && File.Exists(exeCandidate)) {
                                sink.Add(new AppHit(HitType.Exe, scope, exeCandidate, version, pkgType, [Name], 0, evidence));
                            }
                        }
                        catch { }
                    }
        ...
        private static bool AllTokensPresent(string displayNameLower, string[] queryTokens) {
            foreach (var t in queryTokens) {
                if (!displayNameLower.Contains(t)) {
                    return false;
                }
            }
            return true;
        ...
        private static bool PathIsPlausibleDir(string path) {
            try {
                ...
                return !lower.Contains("\\uninstall") && !lower.Contains("%temp%");
            }
            catch { return false; }
        }
        ...
        private static string? ParseExeFromDisplayIcon(string? displayIconRaw) {
            ...
            return fileLower is "uninstall.exe" or "unins000.exe" or "setup.exe" or "msiexec.exe" or "_uninst.exe" ? null : s;
        }
