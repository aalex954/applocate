...
public sealed class SourceRegistryBuilder
{
    ...
    public SourceRegistryBuilder Add(ISource source)
    {
        ...
        EnsureNotBuilt();
        if (!_names.Add(source.Name)) throw new InvalidOperationException($"Source '{source.Name}' already registered.");
        _sources.Add(source);
        return this;
    }
    ...
    {
        EnsureNotBuilt();
        int idx = _sources.FindIndex(s => string.Equals(s.Name, name, StringComparison.OrdinalIgnoreCase));
        if (idx < 0) return this;
        var item = _sources[idx];
        _sources.RemoveAt(idx);
        if (newIndex < 0) newIndex = 0;
        if (newIndex > _sources.Count) newIndex = _sources.Count;
    ...
    {
        if (_built) throw new InvalidOperationException("Builder already used to construct a registry.");
    }
}
