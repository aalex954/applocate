    ...
    public sealed class IndexStore : IIndexStore {
        ...
        public IndexFile Load() {
            try {
                ...
                if (file == null || file.Version != IndexFile.CurrentVersion) {
                    return IndexFile.CreateEmpty();
                }
                ...
                return !string.IsNullOrEmpty(file.EnvironmentHash) && !string.Equals(file.EnvironmentHash, currentHash, StringComparison.Ordinal)
                    ? IndexFile.CreateEmpty()
                    : file;
            }
        ...
        public void Upsert(IndexFile file, string normalizedQuery, IEnumerable<AppHit> hits, DateTimeOffset now) {
            ...
            if (recIdx >= 0) {
                file.Records[recIdx] = updatedRecord;
            }
            else {
                file.Records.Add(updatedRecord);
            }
        ...
        public bool TryGet(IndexFile file, string normalizedQuery, out IndexRecord? record) {
            ...
            // Staleness check
            return DateTimeOffset.UtcNow - record.LastRefreshUtc <= _maxAge;
        }
                ...
                if (removeLegacy) {
                    ...
                    if (segs.Length != ExpectedSegments) {
                        ...
                        if (ok && segs[10].Length > 1) {
                            var confPart = segs[10][1..];
                            // Accept parsable double; if not parsable mark invalid.
                            if (!double.TryParse(confPart, out _)) {
                                ok = false;
                            }
                        }
                        if (!ok) {
        ...
        private static string ComputeEnvironmentHash() {
            try {
                ...
                _ = sb.Append(DateTime.UtcNow.ToString("yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture));
                var sm = Environment.ExpandEnvironmentVariables("%ProgramData%\\Microsoft\\Windows\\Start Menu\\Programs");
                if (!string.IsNullOrEmpty(sm) && Directory.Exists(sm)) {
                    try { _ = sb.Append(new DirectoryInfo(sm).LastWriteTimeUtc.Ticks); } catch { }
                }
                ...
                if (!string.IsNullOrEmpty(la) && Directory.Exists(la)) {
                    try { _ = sb.Append(new DirectoryInfo(la).LastWriteTimeUtc.Ticks); } catch { }
                }
                // Hash
