...
namespace AppLocate.Core.Ranking {
    ...
    /// </summary>
    /// <summary>
    /// Public ranking helper exposing scoring for <see cref="AppHit"/> instances.
    ...
    { "wt.exe", new[]{"wt","windows terminal","microsoft windows terminal"} },
        ...
        private static double ScoreInternal(string normalizedQuery, AppHit hit, Dictionary<string, double>? signals) {
            ...
            double score = 0;
            var collapsedQuery = new string([.. query.Where(ch => ch is not (' ' or '-' or '.'))]);
            if (collapsedQuery.Length < 2) {
                collapsedQuery = query;
            }

            // 1. Token set similarity (Jaccard) over filename & parent directory names
            var fileName = Safe(() => Path.GetFileNameWithoutExtension(path)?.ToLowerInvariant());
            var dirName = Safe(() => Path.GetFileName(Path.GetDirectoryName(path) ?? string.Empty)?.ToLowerInvariant());
            var tokensQ = Tokenize(query);
            var tokensCand = Tokenize(string.Join(' ', [fileName, dirName]));
            // Secondary punctuation/camel splits: expand token sets without overwriting originals
            if (tokensQ.Count > 0) {
                foreach (var t in tokensQ.ToArray()) {
                    foreach (var exp in ExpandToken(t)) {
                        _ = tokensQ.Add(exp);
                    }
                }
            }
            ...
            if (tokensQ.Count > 0 && tokensCand.Count > 0) {
                ...
                score += add; if (signals != null) {
                    signals["TokenCoverage"] = add;
                }
            }
            else if (lowerPath.Contains(query)) {
                score += 0.15; if (signals != null) {
                    signals["CollapsedSubstring"] = 0.15;
                }
            }
            ...
            if (tokenCoverage == 0) {
                ...
                if (!string.IsNullOrEmpty(collapsedQuery) && collapsedName.Contains(collapsedQuery) && !fileName!.Equals(query, StringComparison.OrdinalIgnoreCase)) {
                    score += 0.08; if (signals != null) {
                        signals["CollapsedSubstring"] = 0.08;
                    }
                }
            }

            var extraTokenCountForCandidate = 0; // track tokens not in query for later noise penalty
            if (tokensCand.Count > 0) {
                foreach (var t in tokensCand) {
                    if (!tokensQ.Contains(t)) {
                        extraTokenCountForCandidate++;
                    }
                }
            }
            ...
            if (tokensQ.Count > 0 && tokensCand.Count > 0) {
                ...
                var inter = 0; foreach (var t in tokensQ) {
                    if (tokensCand.Contains(t)) {
                        inter++;
                    }
                }
                ...
                if (union.Count > 0) {
                    ...
                    if (jaccard is > 0 and < 1) {
                        var noiseFactor = 1.0;
                        if (extraTokenCountForCandidate >= 2) {
                            noiseFactor = 0.6;
                        }

                        if (extraTokenCountForCandidate >= 4) {
                            noiseFactor = 0.4;
                        }

                        var add = jaccard * 0.08 * noiseFactor; score += add; if (signals != null) {
                            signals["PartialTokenJaccard"] = add;
                        }
                    }
                }
            }
            ...
            if (!string.IsNullOrEmpty(fileName)) {
                if (fileName.Equals(query, StringComparison.OrdinalIgnoreCase)) {
                    score += 0.30; if (signals != null) {
                        signals["FilenameExactOrPartial"] = 0.30;
                    }
                }
                else if (AliasEquivalent(query, fileName, out var alias)) {
                    score += 0.22; if (signals != null) {
                        signals["AliasEquivalence"] = 0.22;
                    }
                }
                else if (tokenCoverage == 0 && fileName.Contains(query)) {
                    score += 0.12; if (signals != null) {
                        signals["FilenameExactOrPartial"] = 0.12;
                    }
                }
            }
            ...
            if ((hit.Type == HitType.Config || hit.Type == HitType.Data) && !string.IsNullOrEmpty(dirName)) {
                if (dirName.Equals(query, StringComparison.OrdinalIgnoreCase)) {
                    score += 0.20; if (signals != null) {
                        signals["DirAlias"] = 0.20;
                    }
                }
                else if (AliasEquivalent(query, dirName!, out var dirAlias)) {
                    score += 0.18; if (signals != null) {
                        signals["DirAlias"] = 0.18;
                    }
                }
            }
            ...
            var ev = hit.Evidence;
            if (ev != null) {
                var shortcut = ev.ContainsKey("Shortcut");
                var process = ev.ContainsKey("ProcessId");
                ...
                if (signals != null) {
                    if (evidenceAdds != 0) {
                        signals["EvidenceBoosts"] = evidenceAdds;
                    }

                    if (evidenceSynergy != 0) {
                        signals["EvidenceSynergy"] = evidenceSynergy;
                    }

                    if (evidencePenalties != 0) {
                        signals["EvidencePenalties"] = evidencePenalties;
                    }
                }
            }
            ...
            var tempLike = lowerPath.Contains("\\temp\\") || lowerPath.Contains("/temp/") || lowerPath.Contains("%temp%") || lowerPath.Contains("appdata\\local\\temp");
            ...
            if (signals != null && pathPenalties != 0) {
                signals["PathPenalties"] = pathPenalties;
            }
            ...
            if (tokensQ.Count > 1 && !string.IsNullOrEmpty(fileName)) {
                ...
                if (simplified.Contains(joinedInOrder, StringComparison.OrdinalIgnoreCase)) {
                    var allPresent = true;
                    foreach (var t in tokensQ) {
                        if (!simplified.Contains(t, StringComparison.OrdinalIgnoreCase)) { allPresent = false; break; }
                    }
            ...
            if (sourceCount > 1) {
                double harmonic = 0;
                for (var i = 2; i <= sourceCount; i++) {
                    harmonic += 1.0 / i;
                }

                var norm = harmonic / 0.9; if (norm > 1) {
                    norm = 1;
                }

                var add = norm * 0.18; score += add; if (signals != null) {
                    signals["MultiSource"] = add;
                }
            }
            ...
            score += typeAdd; if (signals != null && typeAdd != 0) {
                signals["TypeBaseline"] = typeAdd;
            }
            ...
            if (!string.IsNullOrEmpty(fileName) && !fileName.Equals(query, StringComparison.OrdinalIgnoreCase)) {
                ...
                if (fuzzy > 0.5 && tokenCoverage < 1) {
                    var add = (fuzzy - 0.5) * 0.12; score += add; if (signals != null) {
                        signals["FuzzyLevenshtein"] = add;
                    }
                }
            }
            ...
            if (hit.Type == HitType.Exe) {
                var fn = Safe(() => Path.GetFileName(hit.Path)?.ToLowerInvariant()) ?? string.Empty;
                var uninstallLike = fn.StartsWith("unins", StringComparison.Ordinal)
            ...
            if (lowerPath.Contains("fl cloud plugins")) {
                var related = query.Contains("fl") || query.Contains("cloud") || query.Contains("plugin");
                if (!related) {
                    score -= 0.35;
        ...
        private static void AddSignal(Dictionary<string, double>? map, string key, double value) {
            if (map == null) { return; }
            map[key] = value;
        }

        private static void AddOrAccumulate(Dictionary<string, double>? map, string key, double delta) {
            if (map == null) { return; }
            map[key] = map.TryGetValue(key, out var existing) ? existing + delta : delta;
        }
        ...
        private static IEnumerable<string> ExpandToken(string token) {
            ...
            void FlushAlpha(List<string> output) {
            ...
            void FlushNum(List<string> output) {
                if (numBuf.Length > 0) {
        ...
        private static ScoreBreakdown BuildBreakdown(Dictionary<string, double> map, double total) {
            double Get(string k) {
